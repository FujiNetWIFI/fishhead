...
.create-d64:
	...
	if [ $$? -eq 0 ] ; then \
		echo "creating $(DISK_FILE) from program $(PROG_BASENAME)" ; \
		if [ -f "$(DISK_FILE)" ] ; then \
			rm "$(DISK_FILE)" ; \
		fi ; \
		cc1541 -q \
			...
			-f "$(PROGRAM)" \
			-P \
			-w $(DIST_DIR)/$(PROG_BASENAME)$(SUFFIX) \
			-H "created by fenrock" \
			$(DISK_FILE) ; \
	else \
...
# All VICE targets use the same program path
VICE_TARGETS := c64 c128 c16 pet plus4 vic20 cbm510 cbm610

$(foreach t,$(VICE_TARGETS), \
  $(eval $(t)_EMU_PROG := $(DIST_DIR)/$(PROG_BASENAME)$(SUFFIX)) \
)
